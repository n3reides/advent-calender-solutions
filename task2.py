...
def main():
    ...
    pw_list = get_hardcoded_input()
    len_pws = len(pw_list)
    key_list: List[None] = [None] * len_pws
    real_pws: List[None] = [None] * len_pws
    print("task 1:")
    tic = time.perf_counter()
    for i in range(len_pws - 1):
        split_pw = pw_list[i].split(':')
        key_list[i] = split_pw[0]
        real_pws[i] = split_pw[1].strip(' ')

    with Pool() as pool:
        results = pool.starmap(compare_rules,
                               [(real_pws[0:249], key_list[0:249]), (real_pws[250:499], key_list[250:499]),
                                (real_pws[500:749], key_list[500:749]), (real_pws[750:-1], key_list[750:-1])])
    toc = time.perf_counter()
    time_elasped = toc - tic
    print('# correct passwords:', sum(results))
    print('time elapsed:', time_elasped, 'secs')
    print("task 2:")
    key_list = [None] * len_pws
    real_pws = [None] * len_pws
    tic = time.perf_counter()
    for i in range(len_pws - 1):
        split_pw = pw_list[i].split(':')
        key_list[i] = split_pw[0]
        real_pws[i] = split_pw[1].strip(' ')
    with Pool() as pool:
        results = pool.starmap(comp_rules_two,
                               [(real_pws[0:249], key_list[0:249]), (real_pws[250:499], key_list[250:499]),
                                (real_pws[500:749], key_list[500:749]), (real_pws[750:-1], key_list[750:-1])])
    toc = time.perf_counter()
    time_elasped = toc - tic
...
def comp_rules_two(pw_list: [str], key_list: [str]) -> int:
    counter = 0
    for i in range(len(key_list) - 1):
        split_key = key_list[i].split(' ')
        get_key_pos = split_key[0].split('-')
        key_pos_one, key_pos_two = int(get_key_pos[0]), int(get_key_pos[1])

        if (pw_list[i][key_pos_one - 1] == split_key[1]):
            counter += 1 if (pw_list[i][key_pos_two - 1] != split_key[1]) else 0
        elif (pw_list[i][key_pos_two - 1] == split_key[1]):
            counter += 1 if (pw_list[i][key_pos_one - 1] != split_key[1]) else 0

    return counter


def compare_rules(pw_list: [str], key_list: [str]) -> int:
    counter = 0
    for i in range(len(key_list) - 1):
        split_key = key_list[i].split(' ')
        get_len_interv = split_key[0].split('-')
        if (int(get_len_interv[0]) <= pw_list[i].count(split_key[1]) <= int(get_len_interv[1])):
            counter += 1
    return counter


def get_hardcoded_input() -> [str]:
    inp_str = '''14-15 v: vdvvvvvsvvvvvfpv
...
16-20 j: vjkjjcjjrjjmtnbjjjnj
    '''
    return [x for x in inp_str.split('\n')[0:-1]]
